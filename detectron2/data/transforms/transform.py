...
class GaussianBlurTransform(Transform):
    def __init__(self):
        self.aug = TT.RandomApply([GaussianBlur([0.1, 2.0])], p=0.2),
...
class RandomGrayscaleTransform(Transform):
    def __init__(self):
        self.aug = TT.RandomGrayscale(p=0.2)
    ...
    def apply_image(self, img):
        ...
        return np.array(self.aug(img_pil), dtype=np.uint8)


# 3
class ColorJitterTransform(Transform):
    def __init__(self):
        self.aug = TT.RandomApply([TT.ColorJitter(0.4, 0.4, 0.2, 0.1)], p=0.5)
