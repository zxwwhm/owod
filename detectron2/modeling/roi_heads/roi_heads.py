...
from detectron2.uno.sinkhorn_knopp import SinkhornKnopp
...
class ROIHeads(torch.nn.Module):
    ...
    def from_config(cls, cfg):
        return {
            ...
            "uno_on": cfg.UNO.UNO_ON,
            "seen_class": cfg.OWOD.PREV_INTRODUCED_CLS + cfg.OWOD.CUR_INTRODUCED_CLS
        }
...
class Res5ROIHeads(ROIHeads):
    ...
    def get_gt_features(self, features, targets):
        proposal_boxes = [x.gt_boxes for x in targets]
        box_features = self._shared_roi_transform(
            [features[f] for f in self.in_features], proposal_boxes
        )
        gt_features = box_features.mean(dim=[2, 3])
        return gt_features

    def get_unknown_features(self, features, proposals):
        boxes = []
        for proposal in proposals:
            proposal_boxes = proposal.proposal_boxes
            gt_classes = proposal.gt_classes
            idx = gt_classes == self.num_classes - 1
            boxes.append(proposal_boxes[idx])

        box_features = self._shared_roi_transform(
            [features[f] for f in self.in_features], boxes
        )
        unknown_features = box_features.mean(dim=[2, 3])
        return unknown_features

    def forward(self, images, features, proposals, targets=None,
                uno_image=None, uno_features=None, uno_targets=None):
        """
        todo 默认uno开启
        See :meth:`ROIHeads.forward`.
        """
        ...
        if self.training:
            first_features = self.get_gt_features(features, targets)
            second_features = self.get_gt_features(uno_features, uno_targets)
            ...
            gt_classes = torch.cat([item.gt_classes for item in targets])
            un_classes = torch.Tensor([self.num_classes - 1] * unknown_features.shape[0]).to(gt_classes.device)
            uno_classes = torch.cat([gt_classes, un_classes])
            
            first = torch.cat([first_features, unknown_features])
        ...
        if self.training:
            ...
            pred_instances, _ = self.box_predictor.inference(predictions, proposals, input_features)
